...
class Reporting:
    ...
    def __init__(self, args: Args):
        ...
        self.task_idx_to_dataset = task_idx_to_name = dict({})
    ...
    def init_tensorboard(self):
        ...
        if self.use_tensorboard:
            self.tensorboard_summary = TensorboardSummary(self.title, path=self.out_dir)

    def trace_train(self, seen_training: int, task_idx: int, train_epoch: int, info: dict):
        set = self._train_trace

        if seen_training not in set:
            set[seen_training] = dict({})
        if task_idx not in set[seen_training]:
            set[seen_training][task_idx] = []

            info["train_epoch"] = train_epoch
        set[seen_training][task_idx].append(info)

    def trace_eval(self, seen_training: int, task_idx: int, train_epoch: int, val_epoch: int,
                   info: dict) -> Tuple[bool, bool]:

        set = self._eval_trace
